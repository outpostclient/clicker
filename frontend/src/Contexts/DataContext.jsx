...
export const DataProvider = ({ children }) => {
  ...
  const fetchBlogListExcludeCurrent = useCallback(
    async (category_id, current_blog_id) => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/blogs/category/${category_id}/exclude/${current_blog_id}/`
        );
  ...
  useEffect(() => {
    const fetchCategorysWithBlogs = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8000/api/categories-with-blogs/"
        );
  ...
  useEffect(() => {
    const fetchParentCategorysWithBlogs = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8000/api/parent-categories/"
        );
  ...
  useEffect(() => {
    const fetchCategoryForCategoryFeatureNotNull = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/category-names/")
